  ...
  return (
    <section className="border-b border-zinc-800 p-4">
      <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-500">
        User Site Comparison
      </div>
      <p className="mt-1 font-mono text-[9px] uppercase text-zinc-600">
        Ranked by baseline NPV under common assumptions
      </p>
          ...
          <article key={`${candidate.label}-${candidate.latitude}`} className="border border-zinc-800 p-2.5">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="font-mono text-[9px] text-zinc-500">#{index + 1}</div>
              ...
              <div className="shrink-0 text-right font-mono text-[10px]">
                ...
                <div className="text-zinc-600">NPV</div>
              </div>
            </div>
            <div className="mt-2 flex justify-between border-t border-zinc-800 pt-2 font-mono text-[9px] text-zinc-500">
              <span>RENT {formatMoney(candidate.monthlyRent)}/MO</span>
              <span>{candidate.mapData.competitors.length} NEARBY</span>
            </div>
          </article>
        ))}
      </div>
      <p className="mt-3 font-mono text-[9px] uppercase leading-4 text-zinc-600">
        Only user-selected sites are compared. Nearby places are observations, not demand proof.
      </p>
    </section>
  );
}
