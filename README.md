...
当前版本面向新加坡小商户和个体经营者，重点回答“这个店址值不值得租，以及还有哪三个真实可核验的位置可以比较”。
...
- 以固定量化模型 60 分 + LLM 专业判断 40 分生成总评分
- 针对新加坡返回 3 个候选开店位置、租金基准、优劣势和公开来源链接
...
- `LEASENS_ONEMAP_ACCESS_TOKEN`（可选，后续接入 OneMap 需要）
...
- `LEASENS_ONEMAP_ACCESS_TOKEN`：OneMap token，预留给新加坡官方地理数据增强
...
完整分析完成后，最终 SSE JSON 中应包含：

- `summary.scoreBreakdown.fixedScore`，最高 60
- `summary.scoreBreakdown.llmScore`，最高 40
- `recommendedLocations`，固定返回 3 个 Singapore 候选位置
