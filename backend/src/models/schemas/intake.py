...
class SpaceIntakeRequest(BaseModel):
    ...
    annual_rent_escalation: float = Field(0, ge=0, le=1)
    annual_revenue_growth: float = Field(0, ge=-1, le=1)
    turnover_rent_rate: float = Field(0, ge=0, le=1)
    opening_ramp_months: int = Field(3, ge=0, le=24)
    discount_rate_annual: float = Field(0.08, ge=0, le=1)
